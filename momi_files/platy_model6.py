...
logging.basicConfig(level=logging.INFO,
                    filename="log_model6platy.log")

sfs = momi.Sfs.load("platy_sfs.gz")

#Modelo 8b - modelo com gargalo e posterior expansão no CEP e AF e migração em todas as direções
...
platy_model6.add_size_param("n_bt", lower=1e3, upper=5e4)
platy_model6.add_growth_param("g_CEP", lower=1e-6, upper=1e-3)
platy_model6.add_size_param("n_bt2", lower=1e3, upper=5e4)
platy_model6.add_growth_param("g_AF", lower=1e-6, upper=1e-3)

#platy_model6.add_size_param("n_AF", lower=5e3, upper=1e6)
#platy_model6.add_size_param("n_CEP", lower=5e3, upper=1e6)

platy_model6.add_leaf("AF", g="g_AF", N=4.88e5)
platy_model6.add_leaf("CEP", g="g_CEP", N=1.28e5)
platy_model6.set_size("CEP", t="tdiv_AF_CEP", g=0, N="n_bt")
platy_model6.set_size("AF", t="tdiv_AF_CEP", g=0, N="n_bt2")
...
#platy_model6.add_size_param("n_Andes", lower=5e3, upper=1e6)
...
results = []
n_runs = 100
platy_model6copy = platy_model2.copy()
for i in range(n_runs):
    print(f"Starting run {i+1} out of {n_runs}...")
    platy_model2.set_params(platy_model2.get_params(),randomize=True)
    results.append(platy_model6copy.optimize(method='L-BFGS-B'))
    lik=platy_model6copy.log_likelihood()
    print(lik)

# sort results according to log likelihood, platyk the best one
best_result = sorted(results, key=lambda r: r.log_likelihood, reverse=True)[0]

platy_model6copy.set_params(best_result.parameters)
best_result
